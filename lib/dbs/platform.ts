...
// const { ARIZE_BACKEND, ARIZE_FRONTEND } = process.env;

export async function checkStoreExist({
  user,
  owner,
  access_token,
  ...session
}: SessionProps) {
  const storeAccessToken = access_token ?? session?.aud;
  const contextString = session?.context ?? session?.sub;
  const storeHash = contextString.split("/")[1] || "";

  await axios.post(
    "http://localhost:8080/big-commerce/check-store",
    {
      storeHash,
